...
[CreateAssetMenu(fileName = "FractalPerlinNoiseGenerator", menuName = "SO/NoiseGenerator/FractalPerlinNoiseGenerator")]
public class FractalPerlinNoiseGenerator : NoiseGenerator
{
    [SerializeField]
    private int seed = 0;
    [SerializeField]
    ...
    {
        float[] noiseValues = GetNormalizedNoiseValues(points);
    ...
    public float[] GetNormalizedNoiseValues(Vector3[] points)
    {
        ...
        for (int octaveNum = 0; octaveNum < octaves; octaveNum++)
        {
            ...
            for (int i = 0; i < points.Length; i++)
            {
                ...
                float perlinValue = noise.pnoise(new Vector2(sampleX, sampleZ), maxPeriod);
                noiseValues[i] += perlinValue * amplitude;
            }
    ...
    {
        float maxValue = noiseValues.Min();
        float minValue = noiseValues.Max();
